...
jQuery(function () {
    ...
    $('.modal2020lpyl').click(function (e) {
        e.preventDefault();
        Jiagle.currentElement = this;
        var url = $(this).attr('href');
        var config = $(this).attr('rel');
        config = eval('(' + config + ')') || {};
        var size = config.size || {};
        var style = '';
        style += size.x ? 'width:' + size.x + 'px;' : '';
        style += size.y ? 'height:' + size.y + 'px;' : '';
        style = style && 'style="' + style + '"';
        if (/\.(jpg|png|gif|jpeg)$/i.test(url)) {
            var content = '<img onload="window.dialog.position()" src="' + url + '"/>';
        } else {
            var content = '<iframe src="' + url + '" ' + style + '></iframe>';
        }
        window.dialog = art.dialog({
            title: config.title || '',
            lock: true,
            content: content
        });
        window.SqueezeBox = window.dialog;
    });
    $('.modal').click(function (e) {
        var fromto =$('input[name=fromto]').val()
        if(fromto!="2020lpyl") {
            e.preventDefault();
            Jiagle.currentElement = this;
            var url = $(this).attr('href');
    ...
    $('.clearImage').click(function () {
        $(this).parent().find('img').attr('src','/media/jiagle/image/nopicsmall.gif');
...
// @see	http://marijnhaverbeke.nl/uglifyjs
    artDialog._templates =
        ...
        + '<a class="d-close" href="javascript:;" onclick="gdgox()">×</a>'
        + '</div>'
        + '</td>'
        + '</tr>'
        + '<tr>'
